...
Le check joue `uv run pytest tests/unit/ -n auto --cov=backend --cov-report=term` (branches
activées, cf. `pyproject.toml [tool.coverage.run]`). Il est **advisory** et sans seuil : il ne
...
- **Rapporter à l'humain, dans `reason`, les seuls fichiers du diff du ticket** : pour chacun, le
  pourcentage mesuré et la liste des lignes et branches non couvertes (colonne `Missing` de la sortie
  `term` avec `show_missing`), lues dans la sortie réelle. Ignore le reste du périmètre mesuré.
- Si un chemin critique de logique métier du ticket (un invariant, une branche d'erreur, un critère
  `SC-`) apparaît non couvert, le nommer explicitement : c'est le signal utile pour la review de
  couverture, qui décide s'il manque un test.
- **Jamais** proposer un `# pragma: no cover`, une exclusion dans `[tool.coverage.*]`, ni un seuil.
