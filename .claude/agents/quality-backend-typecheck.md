...
- **Jamais** un `cast()` de complaisance, un `Any` ajouté, un `# type: ignore` / `# pyright: ignore`,
  ...
  `applicable:false`.
- **Erreur dans `tests/**`** : si le fichier est un **test neuf du ticket** (dans les `testFiles` du
  BRIEF) et qu'un **applier autorisé est présent** (voir le garde-fou fixe ci-dessous), tu peux
  proposer une correction **en ajout** qui rend le type vrai en **renforçant** le test : un narrowing
  qui ajoute une assertion (`assert x is not None` avant l'usage), ou un import / nom qui suit un
  renommage de production. Jamais `cast()`, `Any`, ni `# type: ignore` dans un test. Dans tout autre
  cas (test existant, correction qui retire ou réécrit une assertion, applier absent) →
  `applicable:false` ; dans `reason`, donne la cause probable — typiquement une signature de
  production changée par le ticket que le test n'a pas suivie — pour que l'humain tranche.
