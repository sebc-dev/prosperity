...
<objectif>
Tu es l'agent dédié au check **frontend-typecheck** de la quality gate de ce projet — un seul check, le tien.
...
Le check joue `npm --prefix client run typecheck`, soit `tsc --noEmit` (config `client/tsconfig*.json`).
Aucun autofix : tu es saisi de chaque erreur.

- **Remonter** chaque erreur avec fichier, ligne, code `TSxxxx` et message, tirés de la sortie réelle
  de tsc, en te limitant aux fichiers du diff du ticket.
- **Proposer la correction qui rend le type vrai** : narrowing (`if (!x) return`, garde `in` /
  `typeof`), type de retour ou de prop explicite, `satisfies` pour vérifier une forme sans l'élargir,
  type dérivé du schéma OpenAPI généré (`src/lib/api/schema.d.ts`) plutôt qu'un type redéclaré à la
  main. La correction vise le code de production du ticket, elle est chirurgicale.
- **Jamais** `as any`, `as unknown as T`, `!` d'assertion non nulle sans garde, `@ts-ignore` /
