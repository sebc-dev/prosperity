...
- **Global scope**: `/home/negus/dev/prosperity/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/negus/dev/prosperity/.claude/notes/`)
...
2. Glob `/home/negus/dev/prosperity/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/negus/dev/prosperity/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/negus/dev/prosperity/.claude/notes/` — works in any directory
