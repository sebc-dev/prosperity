...
**Domain:** Self-hosted personal finance management (budgets enveloppes, multi-comptes, synchro bancaire)
...
```xml
<!-- Core (via Spring Boot 4.0.x parent) -->
<parent>
...
```bash
# Core
pnpm install primeng @primeng/themes tailwindcss-primeui
pnpm install echarts ngx-echarts
pnpm install @angular/pwa

# Dev dependencies
pnpm install -D tailwindcss @angular-eslint/schematics prettier
```
...
**Impact majeur identifie pendant la recherche.**

Le projet (ADR-0001, SPEC.md, discovery.md) indique Liquibase 5.0.x pour les migrations. Or, Liquibase 5.0 (sorti oct 2025) a change de licence vers FSL (Functional Source License), qui n'est **pas une licence open source**. FSL interdit l'utilisation commerciale concurrentielle pendant 2 ans avant de revenir a Apache 2.0.

**Contrainte projet** : "toutes les dependances MIT ou Apache 2.0".

**Recommandation** : Remplacer Liquibase par Flyway 11.x (Apache 2.0). Flyway est nativement integre avec Spring Boot 4, supporte PostgreSQL, et utilise des migrations SQL pures (plus simple que le XML/YAML Liquibase).

**Migration** : Aucune migration necessaire (projet greenfield). Simplement utiliser Flyway des le depart au lieu de Liquibase.

**ADR-0001 a mettre a jour** pour refleter ce changement.

## Caddy Version Update

Le projet reference Caddy 2.10.x. Caddy 2.10 est sorti en avril 2025, mais la version courante est **2.11.1** (mars 2026). Caddy etant rolling release (seule la derniere version est supportee), il faut cibler 2.11.x.
...
---
*Stack research for: Prosperity -- self-hosted personal finance management*
*Researched: 2026-03-28*
