...
**Restant R et conservation multi-transactions** : R est calculé sur les seules transactions **strictement antérieures** à T dans l'ordre total `(date, id)` (`R = montant du budget − consommation des tx où (date, id) < (date_T, id_T)`). L'excédent est ainsi **additif et conservatif** : `Σ E = max(0, ΣM − budget)` sur une période — deux dépenses qui se partagent un même dépassement ne le génèrent pas chacune en double. L'ordre `(date, id)` est figé (agrégat immuable, ADR 0001) ⇒ matérialisation déterministe et idempotente, indépendante de l'ordre de confirmation. _Limite V1_ : éditer un champ d'une tx qui change sa consommation (catégorie, bascule `force_full_debt`) ne re-matérialise que cette tx, pas ses voisines de période ⇒ leur overflow peut rester périmé jusqu'à leur propre re-matérialisation ; la re-matérialisation de masse d'une période relève de S11.4 (reclassement).
