...
**Cas sans budget actif** : si **aucun budget** ne couvre la catégorie C, il n'y a pas de restant R à absorber → la base de la dette devient le **montant entier** (`base = M`, soit `E = M` dans la notation ci-dessus, équivalent à `force_full_debt`). Une dépense `default` sur compte commun **non budgétée** génère donc une dette de quote-part sur tout le montant ; ajouter un budget couvrant après coup re-matérialise la dette à la baisse (cf. F10 reclassement, S11.4).
