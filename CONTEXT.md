...
Pour une dette `shared_account_overflow` (compte commun, non sensible), les deux colonnes restent visibles **des deux côtés** sur le canal sync : le débiteur les reçoit déjà via `account_shared` (membre du compte commun source), donc le masquage débiteur inconditionnel n'y retire rien, et le créancier les reçoit aussi (cf. §Column-level filter, delta assumé vs canal REST).
...
**`BankingProvider`** :
Protocol pull-only à 3 méthodes (`list_accounts`, `fetch_transactions`, `consent_status`) implémenté par `EnableBankingProvider` et `MockProvider`. **Instancié et appelé uniquement par le job de polling** du module `banking`. Toute exception levée est typée (hiérarchie `BankingProviderError` : `ConsentExpiredError`, `ConsentRevokedError`, `ProviderUnavailableError`, `RateLimitedError`, `IncompatibleAccountError`). ⚠️ `OFXProvider` n'implémente **PAS** ce Protocol (cf. *Import OFX* ci-dessous et ADR 0009) : c'est un parser fichier statique, sans polling ni consentement.
...
**Import OFX** :
`OFXProvider` est un **parser fichier statique synchrone** (`parse(file_bytes) → ParsedOFX(accounts, transactions, encoding_confidence)`), **distinct** du Protocol pull-only `BankingProvider` — il n'en est **pas** une implémentation (pas de polling, pas de consentement). Il **partage** seulement le modèle commun `BankTransaction` et la **base `BankingProviderError`** avec le futur `EnableBankingProvider`. Parser via `ofxparse` (avec wrapper défensif qui catch et re-raise en `BankingProviderError` typées : `IncompatibleAccountError`, `ProviderUnavailableError`, `EncodingDetectionError`) — supporte OFX 1.x SGML + OFX 2.x XML dès MVP. Détection d'encoding déterministe : BOM-first, puis tentative UTF-8 strict, puis fallback windows-1252. **Fallback windows-1252 = "détecté avec confiance" à False** → preview obligatoire (cf. critères F04). Dedup par **hash composite** `(account_id, date, amount, libellé_normalisé)`, FITID OFX ignoré (instable côté ASPSP français). Compte OFX non lié = preview obligatoire avec étape "lier à un compte interne existant" ou "créer le compte interne", **jamais de création automatique**. Mapping persistant dans `bank_account_external_refs`.

**Analyse d'import (S12.3)** : `banking.public.analyze_import(session, parsed_ofx, *, provider, reference_date) → ImportPreview` est **read-only** (aucune écriture ; la création de `Transaction` + l'insert `imported_transactions` sont au commit S12.4, composition root). Le hash de dedup d'une `BankTransaction` sur un compte **interne** est calculé par `compute_import_hash(account_id, tx)` (`banking.public`) = `import_hash(account_id, date, amount_cents, libellé normalisé)` — **source unique** réutilisée à l'identique par le commit S12.4.3 (sinon dedup incohérente). Le libellé = `description` (mémo) sinon `payee` ; `normalize_label` (`backend/shared/bank_labels.py`) est **partagé** avec le futur `MatchScorer` de `reconciliation` (cohérence dedup ↔ matching). L'`ImportPreview` expose `tx_count`, `duplicate_count`, `encoding_confidence`, `date_min`/`date_max`, `amount_max_cents`, les **5 critères F04** (`AutoValidationCriteria` : aucun doublon ∧ encoding high ∧ toutes les tx dans ±3 ans ∧ |montant| ≤ 10 000 € ∧ < 50 tx) et l'agrégat `auto_validatable` (**informatif** : la décision auto-commit/preview appartient à la route/au client). Table de dedup : `imported_transactions` (`import_hash` UNIQUE = backstop d'idempotence du commit).

**INV-S12.3-PREVIEW-ACCESS** : `account_not_linked` et `duplicate_count` sont calculés sur des `internal_account_id` **BRUTS** (résolus depuis un fichier importé via `find_internal_account`), **non filtrés par accessibilité** — pendant exact de l'INV-S12.1-ACCESS. L'`ImportPreview` est donc un **oracle** potentiel (confirme qu'une `external_ref` est liée / qu'une ligne existe déjà sur un compte). La route S12.4 **DOIT** refuser tout `external_ref` résolvant vers un `internal_account_id ∉ accessible_account_ids(user_id)` **avant** exposition, et rendre « lié-mais-inaccessible » **indistinguable** de « non-lié » (non-disclosure uniforme) — sinon un membre forge un OFX portant la réf d'un compte personnel d'un autre membre (F03, « no admin exemption ») pour sonder son existence/contenu.

**Routes d'import (S12.4)** : le parcours HTTP vit au **composition root** `backend/transports/imports_http.py` — **hors `backend.modules`** (comme `main.py`). C'est imposé par le graphe directionnel (ADR 0005) : `banking ⊥ transactions` sont des modules **pairs** (interdits de s'importer, pas même leur `.public`), or le `commit` consomme `banking.public` (parse/analyse/log) **et** `transactions.public` (création de la `Transaction`). Aucun module ne peut faire les deux ; seul le composition root importe librement toutes les `.public`. Le **contrat import-linter #6** (« transports consumer-only », miroir du #5 MCP) interdit à tout `backend.modules.*`/`backend.shared` d'importer `backend.transports` — il consomme, jamais consommé. `banking` n'importe **jamais** `transactions` ; aucune `ignore_imports` neuve. Trois routes (`/imports/ofx`) : `POST /preview` (analyse read-only, gate INV-S12.3-PREVIEW-ACCESS → 422 `account_not_linked` si non-lié/inaccessible), `POST /link-account` (crée le mapping `bank_account_external_refs` après gate d'accessibilité → 404 non-disclosure si compte inaccessible, **jamais** de création de compte interne), `POST /commit` (re-upload stateless du fichier).

**Commit en `draft` (S12.4)** : le `commit` crée **une `Transaction` `draft` par ligne non dupliquée** (`create_draft` + `add_split` jambe `funding` seule : `account_id` = compte interne, `amount_cents`/`currency` de la ligne, `category_id` NULL). Il **reste `draft`** — jamais `planned`/`confirmed` (`transition_to_planned` impose `assert_zero_sum` : une jambe `funding` seule est déséquilibrée ; l'utilisateur catégorise ensuite manuellement en ajoutant une jambe `classification`). Aucune catégorie « Sans catégorie » fabriquée (E06). Gates : compte cible accessible (404), chaque réf du fichier liée à CE compte (422 `account_not_linked`), toutes les devises == `household.base_currency` (422 `currency_mismatch`, rejet global — c'est le boundary qui crée la `Transaction` qui valide la devise, ADR 0008). Dedup en deux temps : `known_import_hashes` (un `SELECT … IN`, partagé avec `analyze_import`) + un set intra-batch (doublons inter-fichiers **et** intra-fichier) ; `UNIQUE(import_hash)` = backstop. **Idempotent** : re-commit du même fichier ⇒ `created=0`. **Aucun `commit()` dans le code** : la frontière est `get_db` (ADR 0015) → l'import est **atomique** (une ligne qui échoue → rollback total, pas d'import partiel). `record_imported`/`known_import_hashes` sont des primitives `banking` flush-only re-exportées par `banking.public` (le journal `imported_transactions` reste propriété de `banking` ; le composition root ne touche jamais `banking.models`). Cap `Content-Length` → 413 avant lecture du corps (défense en profondeur, miroir de `MAX_OFX_BYTES`). `user_overrides` : champ accepté mais **no-op** en V1 (catégorisation manuelle).
...
Pièce centrale du module `modules/sync/`. Reçoit le batch de mutations PowerSync, dispatch chaque mutation au sous-handler de sa table (`handlers/transactions.py`, etc.) qui appelle les `service.py` des modules métier via leur `public.py` (jamais d'écriture DB directe). Séquence stricte par mutation **dans une transaction DB unique** (sauf délivery email/push qui sort en `BackgroundTasks` post-commit) : auth & RBAC → idempotence check via `client_request_id` (scopé user, cf. §`client_request_id`) → Pydantic validation → domain validation → DB write → **matérialisation synchrone des projections** (dettes via `DebtCalculator`, cf. ADR 0002) → publication d'events sur le mini-bus → **append `sync_request_log` (dans la même transaction que le write)** → **commit** → ack. L'append est **atomique** avec le write (S13.6 / delta D-B : « commit → append » lu à la lettre laisserait une fenêtre crash → double-write au replay). Frontière transactionnelle **par mutation** : échec de N → rollback de N seul, 1..N-1 restent committées. Cf. ADR 0014.

**`client_request_id`** :
UUID généré localement par le client à chaque mutation (UUID **v7 recommandé** pour l'ordonnancement temporel), garantit l'idempotence en cas de retry après timeout réseau. **Le serveur accepte tout UUID bien formé** (delta **D7** S13.2 : contraindre la version couplerait le serveur à l'implémentation client ; la v7 reste la recommandation client). Persisté dans `sync_request_log` (server-only, retention 30j) sous une **PK composite `(user_id, client_request_id)`** → idempotence **scopée par user** (l'unicité composite ferme la pré-emption / l'oracle cross-user, review Sécu F1).

**`WriteResult`** :
Réponse PowerSync par mutation, format `{success: bool, error?: {code, message}, server_values?: {…}}`. Codes d'erreur d'un **vocabulaire FERMÉ** (`WriteErrorCode` `Literal`, S13.6 / D6) : `validation_error`, `immutable_field_violation`, `unbalanced_transaction`, `invalid_state_transition`, `uncategorized_expense`, `not_found`, `auth_denied`, `unknown_table`, `not_implemented_yet`. La table de correspondance exception domaine → code est la **source unique** `sync.service.errors.to_write_error` ; `message` est **statique par code** (jamais `str(exc)`/PII). Erreurs récupérables → client purge la mutation locale ; erreurs serveur **non mappées** (500) → PowerSync retry automatique. `server_values` reporte les **IDs générés serveur** d'un `insert` (étape 10) que le client adopte.

**Bucket** :
Unité de découpage de la synchronisation PowerSync (flow download). Chaque bucket est paramétré par une identité (user_id, account_id, budget_id, debt_id) ou est **global** (`household`), et regroupe les rows qu'un appareil de ce périmètre a le droit de recevoir. Les sync rules PowerSync sont **déclaratives et mono-table** (pas de JOIN ni de sous-requête cross-table dans une requête `data`) — les six familles matérialisées (`powersync/sync_rules.yaml`, S13.7) en découlent :
- `user_personal` (param `account_id` = `accounts WHERE owner_id = user`) : comptes personnels + leurs transactions/splits. *(notifications/savings reportés — tables inexistantes.)*
- `account_shared` (param `account_id` = `account_members WHERE user_id = user`) : comptes communs + transactions/splits + `account_members`.
- `user_budget` (param `budget_id` = `budget_contributors WHERE user_id = user`, **D-BUD**) : budgets perso **et** communs, routés par **contributeur** (la table `budgets` n'a pas d'`account_id`, donc un budget commun ne peut pas être routé vers `account_shared`).
- `user_debt` (param `user_id`) : dettes et share_requests dont l'utilisateur est partie, avec **masquage de colonnes par destinataire** (voir Column-level filter).
- `user_settlement` (param `debt_id` = dettes de l'utilisateur, **D-SET**) : `settlement_lines` (mono-hop par `debt_id`). `settlements` n'est **pas** publié (fail-closed : `note` PII + routage per-participant inexprimable mono-table) → reste REST-only.
- `household` (**global**, tout user authentifié) : référentiels foyer (`categories`) + `users_public`.
...
**`users_public`** :
Projection NON-PII de l'identité utilisateur synchronisée au foyer (bucket `household`) : `user_id`, `display_name`, `role`. Permet d'afficher "demandé par Alice" sans exposer `email`/`password_hash`. **Maintenue par un trigger Postgres** (`sync_users_public`, S13.7 / D-UP, gabarit `admin_audit_logs`) qui upsert la projection à chaque INSERT/UPDATE de `users.display_name`/`users.role` — robuste à tout chemin d'écriture, zéro couplage au service auth. `avatar_url` reporté (colonne absente de `users`).
...
Mécanisme où la requête de bucket sélectionne explicitement `NULL AS <column>` pour masquer un champ selon le destinataire. Utilisé pour **`source_transaction_id` ET `account_id`** (deux colonnes, ADR 0003 maj #145) sur les dettes : le bucket `user_debt` porte **deux requêtes `data` sur `debts`** — la vue créancier (`WHERE to_user_id = user`) projette les colonnes réelles, la vue débiteur (`WHERE from_user_id = user`) projette `NULL` sur les deux colonnes **inconditionnellement** (**D-MASK**, fail-safe : pas de `CASE`, donc aucun fail-open sur un futur `origin`). Le CHECK `no_self_debt` garantit qu'un user est créancier **xor** débiteur d'une row → pas de collision intra-bucket. Idem `share_requests` côté débiteur (`requested_from`) pour `source_transaction_id` (**D-SR**).
La vue **créancier** est elle aussi inconditionnelle (colonnes réelles pour tout `origin`, overflow inclus) : **delta assumé vs canal REST** (`GET /debts` masque encore les deux colonnes pour les dettes overflow en V1, en attendant la logique d'ownership E11). C'est sans fuite car le créancier d'une dette overflow est membre du compte commun source et reçoit déjà ce compte + sa transaction source via le bucket `account_shared` — le canal sync ne révèle rien que `account_shared` ne portait déjà. Garder la vue créancier inconditionnelle est ce qui permet à D-MASK de rester sans `CASE` des deux côtés. En profondeur de défense, `materialization_trace` (server-only) est exclu **à la publication** (column-list, **D-MAT**) — jamais répliqué.
