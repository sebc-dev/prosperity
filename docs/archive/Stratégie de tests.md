...
> Document de synthèse de la stratégie de test retenue pour l'application personnelle de gestion de comptes, budget et épargne.
> 
> **Date** : mai 2026 **Statut** : Recommandation — à valider avant implémentation **Niveau de confiance global** : Moyen-Haut **Documents liés** : `Architectures BS.md`, `Stack BS v1.md`, `Enable Banking Oy.md`, `Pydantic.md`
...
**Périmètre du document** : stratégie globale, choix d'outils, conventions de structure, pipeline CI. Le contenu exact des tests par module est traité dans les documents de spécification de chaque module (`transactions`, `debts`, `reconciliation`…).

---

## 2. Principes directeurs

1. **L'effort de test va là où il y a des invariants à protéger.** Les modules à `domain.py` (`transactions`, `reconciliation`, `debts`, `forecasting`) sont sur-investis. Les CRUD enrichis (`budget`, `savings`, `auth`, `accounts`) sont testés sobrement.
    
2. **Les tests sont le filet face à Claude Code.** Un assistant qui génère beaucoup de code rapidement amplifie la surface d'erreur silencieuse. La suite de tests doit pouvoir affirmer "ce refactor n'a rien cassé" sans relire 800 lignes. Cela oriente l'effort vers les invariants et les tests d'architecture, pas vers la couverture cosmétique.
    
3. **TDD strict sur le domaine, libre ailleurs.** Sur les 4 modules à `domain.py`, on écrit le test d'abord. C'est non négociable parce que les invariants sont mieux pensés en les exprimant comme tests avant d'avoir un biais d'implémentation.
    
4. **Pyramide enrichie d'intégration DB substantielle.** On retient une pyramide classique (beaucoup d'unitaires, peu d'E2E), mais on l'élargit à la base avec une couche d'intégration DB (testcontainers Postgres) substantielle. Le testing trophy pur ne convient pas parce que le domaine financier mérite vraiment ses tests unitaires de domaine.
...
6. **Tests d'architecture comme garde-fou structurel.** L'architecture Modular Monolith ne tient que si la discipline d'import est mécaniquement vérifiée. C'est probablement le test à plus haut ROI face à Claude Code.
    
7. **Ce qui n'est pas testé est documenté.** Les fichiers générés (clients OpenAPI, types TS), les bibliothèques tierces (PowerSync SDK, shadcn/ui non modifiés), les schémas Pydantic d'API (couverts par le typage), le CSS pur ne sont pas testés. C'est un choix, pas un oubli.
...
```
                       ...
                       Tests de contrat Enable Banking
...
**Cible** : `modules/*/domain.py` et `shared/money.py`, `shared/currency.py`.

**Outils** : pytest, Hypothesis, pas de DB, pas de réseau, pas de FastAPI.

**Approche** : TDD strict. Pour chaque règle métier identifiée dans le document de spécification du module, un ou plusieurs tests example-based plus une ou plusieurs propriétés Hypothesis quand l'invariant est quantifiable.

**Exemples d'invariants ciblés** :

- `Money + Money` n'autorise pas le mélange de devises (lève `IncompatibleCurrencyError`)
- Somme des `splits` d'une `Transaction` valide = 0 pour toute combinaison
- Matrice de dettes antisymétrique : `debt(A→B) == -debt(B→A)`
- Reconciliation idempotente : appliquer le matching deux fois donne le même état
- Projection de solde : monotonie (ajouter une transaction future ne diminue jamais le solde projeté à T+1 quand le montant est positif)
...
**Investissement initial** : une `tests/strategies.py` partagée avec les stratégies réutilisables. Estimation : ~150 lignes, ~1 jour.
...
def balanced_splits_strategy(draw, n_splits=None):
    ...
    # génère n-1 splits aléatoires, le dernier compense
    ...
```
...
**Cohabitation property + example** : utiliser `@example()` pour épingler les cas concrets connus en plus de la génération aléatoire. Permet d'utiliser une propriété comme test de régression précis.

### 4.3 Tests d'architecture (import-linter)

**Outil retenu** : **import-linter**, choisi pour sa maturité, son format de config déclaratif TOML, et son support natif des contrats `layers` / `forbidden` / `independence`. Alternatives évaluées et rejetées : PyTestArch (fluent API plus verbeuse), ArchUnitPython (trop jeune, 3 semaines de vie en mai 2026).

**Contrats minimaux à définir dès la phase 2 du backend** :

```toml
# .importlinter — extrait illustratif
...
type = "forbidden"
source_modules = ["backend.modules.*.domain"]
forbidden_modules = [
    "sqlalchemy",
    "fastapi",
    "httpx",
...
[[importlinter.contracts]]
name = "Banking SDK isolation"
type = "forbidden"
source_modules = ["backend"]
forbidden_modules = ["enablebanking", "httpx"]
ignore_imports = [
    "backend.modules.banking.* -> *"
]
```

**Règles structurelles ciblées** :

1. Independence entre modules (pas d'import direct module → module)
2. `domain.py` n'importe que stdlib + Pydantic + `shared/*`
3. `shared/` n'importe rien des modules
4. Pas d'import circulaire (automatique)
5. Seul `modules/banking/` peut importer le client Enable Banking
6. `schemas.py` (Pydantic API) n'importe pas `models.py` (SQLAlchemy)

**Coût** : ~60 lignes de config + 0 ligne de code de test (import-linter s'exécute via CLI). Coût ridicule par rapport au ROI.

### 4.4 Tests d'intégration avec DB

**Cible** : `modules/*/service.py`, repositories, logique transactionnelle.
...
**Performance** : ~10-30 secondes de démarrage pour le container, négligeable sur une session, important sur les tests isolés. Mitigations : `pytest --reuse-db` en local, container partagé en CI.

### 4.5 Tests d'intégration HTTP (endpoints)

**Cible** : routes FastAPI, sérialisation, auth, RBAC.
...
**Cas particulier — write upload handler PowerSync** : tests intégration spécifiques avec scénarios de mutations multiples, conflits, idempotence. C'est le cœur de la résolution server-authoritative. Couverture visée : exhaustive (cf. section 7).
...
**Niveau 1** : test pytest qui prend une DB vide via testcontainers, applique `alembic upgrade head`, et compare le schéma résultant à un schéma de référence (snapshot SQL versionné). Détecte les conneries syntaxiques et les oublis.

**Niveau 2** : pour chaque migration nommée selon la convention `*_backfill.py`, un test pytest dédié qui :

- Restaure une fixture représentative à l'état pré-migration
- Applique `upgrade`
- Vérifie l'état des données via queries d'assertion
- Si `downgrade` est défini, vérifie que `down → up` converge
...
### 4.7 Tests de contrat Enable Banking

Trois niveaux distincts :

**Niveau A — Tests unitaires de l'adaptateur** : mock du client HTTP, vérification que `EnableBankingProvider` produit les bons appels et parse correctement les réponses. Push CI.

**Niveau B — Tests de contrat sur fixtures JSON** : fixtures versionnées dans `tests/fixtures/enable_banking/`, validation par les modèles Pydantic générés depuis l'OpenAPI Enable Banking via `datamodel-code-generator`. Si Enable change un champ obligatoire, les fixtures cessent de valider → CI rouge. Push CI.
...
**Cible** : `components/business/` (TransactionRow, AccountCard, DebtSummary, BalancePanel).
...
**Cible** : `features/` (add-transaction, reconcile-bank, settle-debt, link-bank-account).
...
**Stratégie** : un `MockPowerSyncDatabase` dans `tests/mocks/powersync.ts` qui expose la même surface API que `PowerSyncDatabase` mais s'appuie sur une queue de mutations en mémoire et un état synchronisable manuellement (helpers `simulateOffline()`, `simulateReconnect()`, `simulateSyncedFromServer(rows)`).

**Usage** : tous les tests de features qui dépendent de PowerSync utilisent cette mock. Les tests bout en bout réels sont réservés à Playwright (cf. section 6).
...
|#|Parcours|Couvre|
|---|---|---|
|1|Onboarding multi-user|Login + RBAC + setup multi-user (création comptes personnels + commun + invitation)|
|2|Sync offline → online (golden path)|PowerSync, write upload handler, convergence multi-device|
|3|Reconciliation locale ↔ bancaire|Import OFX, matching, validation, audit trail|
|4|Cycle de dette complet|Création dette depuis transaction non-budgétisée, règlement partiel, règlement total, invariant zero-sum vérifié à chaque étape|
|5|Lien bancaire Enable Banking|OAuth init, callback mocké (pas la vraie SCA), fetch fixtures, import en DB, affichage UI|
...
Trois niveaux distincts qui ne se substituent pas l'un à l'autre :

|Niveau|Cible|Outils|Couverture visée|
|---|---|---|---|
|1 — Server-side|Write upload handler|pytest + testcontainers|Exhaustive|
|2 — Client mocké|Hooks, features|Vitest + MockPowerSyncDatabase|Sélective|
|3 — Bout en bout|Convergence multi-device|Playwright + PowerSync Service en testcontainer|Un seul golden path|
...
- **Idempotence** : rejouer la même mutation N fois = même état final
- **Convergence** : pour toute permutation valide de mutations, l'état final converge selon la règle (last-write-wins par défaut, customisée par endpoint si besoin)
- **Isolation cross-user** : une mutation d'Alice ne peut jamais affecter le compte de Bob
- **Ordering** : timestamps client respectés pour l'arbitrage
...
|Job|Contenu|
|---|---|
|backend-lint|ruff, mypy, import-linter|
...
**Objectif** : < 5 min total via parallélisation. Au-delà, l'effet pavlovien disparaît et tu ne pousses plus.
...
- Notification (Sentry, Discord, peu importe)
...
|`modules/banking/provider.py`|≥ 85% lignes|coverage.py|
...
- Nombre de violations import-linter ignorées (cible : 0)
...
|Architecture|import-linter|2.x|
...
- **Property tautologique.** Reformuler l'implémentation comme propriété sans la tester. Ex. interdit : `assert money.add(other).amount == money.amount + other.amount` (c'est juste la définition). Permis : `assert money.add(other).amount == other.add(money).amount` (commutativité).
- **Désactiver import-linter à la première violation.** Si une règle gêne, c'est soit la règle qui est mal calibrée (corriger la règle), soit l'architecture qui dérive (corriger le code). Pas de `# noqa: contract-1` qui s'accumulent.
...
- **Tests E2E qui dupliquent les tests d'intégration.** Un E2E coûte 50x un test d'intégration. Pas de "test E2E pour vérifier qu'un endpoint répond 200".
...
- **Tests sur les schémas Pydantic d'API.** Le typage les couvre. Tester un `AccountCreate(...)` est inutile.
...
Ordonnée pour s'inscrire dans la roadmap d'implémentation globale du `Stack BS v1.md`.

### Phase 2 backend (bootstrap)

- [ ] Installer pytest, pytest-asyncio, Hypothesis, coverage, testcontainers-python, factory-boy, import-linter
- [ ] Créer `tests/conftest.py` avec fixtures `db_session`, `client`
- [ ] Créer `.importlinter` avec les 5 contrats minimaux
- [ ] Créer `.pre-commit-config.yaml` avec ruff + mypy
- [ ] Premier test bidon `tests/unit/test_smoke.py` pour valider le pipeline

### Phase 2 backend (modules à domain)

- [ ] Pour le module `transactions` (premier module traité) : structure de test complète comme archétype
    - `tests/unit/domain/test_transactions.py` (TDD, property-based)
    ...
    - `tests/strategies.py` (transaction_strategy, money_strategy)
- [ ] Réplication du pattern sur `debts`, `reconciliation`, `forecasting`

### Phase 2 backend (Enable Banking)

- [ ] Génération du client Pydantic depuis l'OpenAPI avec `datamodel-code-generator`
- [ ] Capture des fixtures initiales (sandbox Enable Banking)
- [ ] Tests niveau A + B en push CI
- [ ] Script `refresh_enable_banking_fixtures.py`

### Phase 3 client

- [ ] Installer Vitest, Testing Library, MSW
- [ ] Créer `tests/mocks/powersync.ts`
...
## Annexe — Synthèse des décisions

|#|Sujet|Décision|
|---|---|---|
|1|Philosophie|Pyramide enrichie d'intégration DB substantielle, pas de testing trophy pur|
|2|TDD|Strict sur les 4 `domain.py`, libre ailleurs|
|3|Property-based testing|Hypothesis dès J1, scope = `domain.py` + `shared/money.py` + write upload handler|
|4|Stratégie PowerSync|3 couches : exhaustif serveur, mocké client, 1 E2E|
|5|Tests d'architecture|import-linter, 5-6 contrats, J1|
|6|Contract Enable Banking|Unitaires + fixtures en push CI ; appels réels en nightly sur compte dédié|
|7|Parcours Playwright|5 : onboarding multi-user, sync offline→online, reconciliation, cycle dette, lien bancaire|
...
> **La stratégie de test est l'extension naturelle de l'architecture modulaire** : on protège fort les invariants des 4 modules à `domain.py`, on protège raisonnablement les services, on protège mécaniquement la structure d'import, on n'investit pas dans les CRUD.
> 
...
1. **Valider ce document** : passer en revue les décisions, identifier les désaccords éventuels avec les choix d'archi.
2. **Détailler le module `transactions` comme archétype testé** : structure complète de tests qui sert de modèle pour les 6 autres modules.
3. **Créer le skill `finance-domain-model`** : ce skill consolidera les invariants du domaine financier, qui sont aussi les propriétés Hypothesis à tester. Le doc et le skill se nourrissent l'un l'autre.
4. **Bootstrap CI** : créer `.github/workflows/push.yml` et `.github/workflows/nightly.yml` avec les jobs définis section 9, même vides au début.
5. **Documenter dans `CLAUDE.md`** : conventions de nommage des tests, structure des dossiers, commandes principales (`pytest`, `pytest --hypothesis-seed=0`, `vitest run`, `playwright test`).