...
| **P11.4.3** | **`transactions` — propagation du reclassement** (scindé du P11.4.3 d'origine, delta D-A) : éditer `category_id` propage la nouvelle catégorie sur la **jambe `classification`** (source de vérité consommation/overflow E08.5, sinon no-op) + enrichit l'event d'un `previous_category_ids` (ancienne catégorie, seule valeur non relisible) + note ADR 0001. Tests : propagation jambe, consommation suit, pas d'`ImmutableFieldViolation` | ~80 |
| **P11.4.4** | **`debts` — guard `category_id` + voisines** : `rematerialize_overflow_on_edit` ne réagit en S11.3 qu'à `debt_generation_override` ; étendre à `category_id` (sélectionne le budget couvrant, donc le restant, donc la base) et re-matérialiser les **voisines de période** (ancien & nouveau budget) dont le restant est décalé — réutilise le chemin idempotent P11.4.1 (jamais une 2ᵉ voie). Lève la `CONTEXT.md` §Excédent _Limite V1_. Tests : reclasser une tx → overflow recalculé sur le nouveau budget (ou retiré si non budgétée) ; voisines re-matérialisées ; idempotence ; `share_request` intacts | ~120 |
