...
<div class="mx-auto max-w-4xl py-2">
	...
	<div class="mb-6 flex gap-1 overflow-x-auto border-b border-gray-200 dark:border-gray-800 md:hidden">
		{#each sections as section}
			{#if section.show}
				<a
					href={section.href}
					class="whitespace-nowrap border-b-2 px-4 py-2 text-sm font-medium transition-colors {currentPath.startsWith(section.href)
				...
				{#each sections as section}
					{#if section.show}
						<li>
							<a
								href={section.href}
								class="block rounded-md px-3 py-2 text-sm font-medium transition-colors {currentPath.startsWith(section.href)
